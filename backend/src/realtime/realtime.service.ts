...
export class RealtimeService {
  ...
  emitFriendRequestAccepted(toUserId: string, payload: FriendRequestResolvedEvent) {
    this.emitToUser(toUserId, "friend_request.accepted", payload);
  }

  emitFriendRequestDenied(toUserId: string, payload: FriendRequestResolvedEvent) {
    this.emitToUser(toUserId, "friend_request.denied", payload);
  }

  emitFriendRequestCanceled(toUserId: string, payload: FriendRequestResolvedEvent) {
    this.emitToUser(toUserId, "friend_request.canceled", payload);
  }

  emitFriendshipDeleted(userIds: [string, string], payload: FriendshipDeletedEvent) {
    const [a, b] = userIds;
  ...
  emitDirectMessageCreated(userIds: [string, string], payload: DirectMessageCreatedEvent) {
    if (!this.server) return;
    const [a, b] = userIds;
