...
export class TetrisGame {
  ...
  tick(): boolean {
    ...
    if (this.canMove(this.currentPiece, 1, 0)) {
      ...
      log(`Locking piece ${this.currentPiece.type} at row=${this.currentPiece.row} col=${this.currentPiece.col}`);
      this.lockPiece();
      this.clearLines();
      if (!this.spawn()) {
  ...
  private spawn(): boolean {
    ...
    if (blocked) {
      ...
      log(`Spawned ${this.currentPiece.type} at row=${this.currentPiece.row} col=${this.currentPiece.col}`);
    }
    return !blocked;
  }
  ...
  private clearLines(): void {
    ...
    for (let r = BOARD_ROWS - 1; r >= 0; r--) {
      if (this.board[r].every((cell) => cell !== 0)) {
        ...
        this.board.unshift(Array.from<string | 0>({ length: BOARD_COLS }).fill(0));
        cleared++;
        r++;
      }
    }
  ...
  private hardDrop(): void {
    ...
    log(`Hard-drop piece ${this.currentPiece.type} to row=${this.currentPiece.row}`);
    this.lockPiece();
    this.clearLines();
    if (!this.spawn()) {
