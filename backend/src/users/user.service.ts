...
export function create2FaSession(userId: string) {
  ...
  run(
    "INSERT INTO users_2fa (id, userId, secret, expiredAt) VALUES (?, ?, ?, ?)",
    twoFaId,
    userId,
    twoFaSecret,
    expiredAt,
...
export function isValidTwoFaToken(
  twoFaSecret: string,
  twoFaId: string,
  userId: string,
  token: string,
) {
  const result = get<TwoFARow>(
    "SELECT * FROM users_2fa WHERE id = ? AND userId = ? AND secret = ?",
    twoFaId,
    userId,
    twoFaSecret,
  );
  ...
  if (
    ...
    !user.twoFaEnabled ||
    !user.twoFaSecret ||
    new Date(result.expiredAt) < new Date()
  ) {
