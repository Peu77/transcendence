...
import {
    create2FaSession,
    createUser, deleteProfilePicture,
    ensureUploadDir,
    findUserByEmail, getUserById,
    isValidTwoFaToken, updateUserProfilePictureId,
...
async function uploadProfilePictureHandler(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (file.mimetype && !file.mimetype.startsWith('image/'))
        return reply.code(HttpStatusCode.BAD_REQUEST).send({error: 'Invalid file type. Only images are allowed'});

    const user = getUserById(request.userId!!);
    if (!user)
        return reply.code(HttpStatusCode.NOT_FOUND).send({error: 'User not found'});
    ...
    if (!uploadedPicture)
        return reply.code(HttpStatusCode.INTERNAL_SERVER_ERROR).send({error: 'Failed to upload profile picture'});

    await updateUserProfilePictureId(user.id, newProfilePictureId);
    return reply.send({message: 'Profile picture uploaded successfully', profilePictureId: newProfilePictureId});
}
