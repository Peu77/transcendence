...
export class FriendsController {
  ...
  async accept(@UserId() userId: string, @Param("requestId") requestId: string) {
    const friendship = await this.friendsService.acceptRequest(requestId, userId);
  ...
  async cancel(@UserId() userId: string, @Param("requestId") requestId: string) {
  ...
  @Patch("presence")
  async updatePresence(@UserId() userId: string, @Body() body: UpdatePresenceDto) {
    const presence = await this.friendsService.updateMyPresence(userId, body.status);
    return {
  ...
  async sendMessage(
    ...
    const msg = await this.friendsService.sendDirectMessage(userId, friendUserId, body.content);
    return {
      id: msg.id,
  ...
  async getMessages(
    ...
    const result = await this.friendsService.getDirectMessages(userId, friendUserId, {
      limit: query.limit,
      before: query.before,
      after: query.after,
    });
