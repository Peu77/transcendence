  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#666", fontSize: "0.9rem" }}>
        Last updated:{" "}
        {lastUpdated.current
          ? lastUpdated.current.toLocaleTimeString()
          : "never"}
      </p>
    </div>
  );
...
function Blinker() {
  ...
  return <span style={{ color: on ? "green" : "gray" }}>{on ? "●" : "○"}</span>;
}
      ...
      {pokemon && (
        ...
        >
          ...
          <div>
            ...
            <div style={{ color: "#555" }}>
              Types: {pokemon.types.map((t) => t.type.name).join(", ")} | H:{" "}
              {pokemon.height} | W: {pokemon.weight}
            </div>
          </div>
        </div>
      )}
...
function TodoList() {
  ...
  const add = () => {
    ...
    if (!t) return;
    setTodos((list) => [
      { id: nextId.current++, text: t, done: false },
      ...list,
    ]);
    setText("");
  };
  const toggle = (id: number) =>
    setTodos((list) =>
      list.map((td) => (td.id === id ? { ...td, done: !td.done } : td)),
    );
