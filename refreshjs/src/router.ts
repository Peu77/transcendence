...
function notify(path: string) {
  for (const l of Array.from(listeners)) l(path);
}
...
export function Router({
  ...
  basename = "",
  notFound = null,
}: RouterProps): VNode | null {
  const [path, setPath] = useState<string>(() =>
    stripBasename(currentPath(), basename),
  );
...
function matchLevel(
  ...
  if (!routes || routes.length === 0)
    return segs.length === 0 ? { chain: [], consumed: 0 } : null;
  ...
  for (const r of routes) {
    ...
    if (remaining.length === 0) {
      const idxRes = matchLevel(
        r.children?.filter((c) => !!c.index),
        remaining,
        params,
      );
      if (idxRes && idxRes.chain) {
        return {
          chain: [{ route: r, params }, ...idxRes.chain],
          consumed: m.consumed + idxRes.consumed,
        };
      }
      ...
      if (childRes) {
        return {
          chain: [{ route: r, params }, ...childRes.chain],
          consumed: m.consumed + childRes.consumed,
        };
      }
    }
  }
  ...
  if (star) {
    return {
      chain: [{ route: star, params: accParams }],
...
function consume(
  routeSegs: string[],
  segs: string[],
): { consumed: number; params: Params; wildcard: boolean } | null {
  const out: Params = {};
