...
export function TableTennis3D({ onTableClick }: TableTennis3DProps) {
  ...
  return h('div', { class: 'flex items-center justify-center min-h-[400px] perspective-1000' },
    h('div', {
      class: `relative transform-style-preserve-3d transition-all duration-300 cursor-pointer ${isHovered ? 'scale-110 rotate-y-3' : ''}`,
      ...
      h('div', {
        ...
        // Court Lines
        h('div', { class: 'absolute left-1/2 top-4 bottom-4 w-0.5 bg-white opacity-40 transform -translate-x-1/2' }),
        h('div', {
        ...
        h('div', {
          class: 'absolute w-3 h-16 bg-gray-600',
          style: { right: '10%', top: '100%', transform: 'translateZ(-32px)' }
        }),
        h('div', {
          class: 'absolute w-3 h-16 bg-gray-600',
          style: { left: '10%', bottom: '0%', transform: 'translateZ(-32px) translateY(100%)' }
        }),
        h('div', {
          class: 'absolute w-3 h-16 bg-gray-600',
          style: { right: '10%', bottom: '0%', transform: 'translateZ(-32px) translateY(100%)' }
        })
      ),
      ...
      // Ball
      h('div', {
        class: `absolute w-3 h-3 bg-white rounded-full shadow-lg transition-all duration-300 ${isHovered ? 'animate-ping' : ''}`,
        style: {
          left: '45%',
          top: '40%',
    ...
    // Click to Play Text
    h('div', { class: 'absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-full mt-8' },
      h('div', { class: 'text-center' },
        h('div', { class: 'text-cyan-400 font-mono tracking-wider animate-pulse' }, '» CLICK TO ENTER GAME «'),
        h('div', { class: 'text-xs text-gray-400 mt-1 font-mono' }, 'Press SPACE or click the table')
      )
    )
  );
}