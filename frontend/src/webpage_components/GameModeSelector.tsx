      ...
      {gameModes.map(mode => (
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <div className={`text-xs font-mono ${getDifficultyColor(mode.difficulty)} bg-gray-800 px-2 py-1`}>
              {mode.difficulty}
            </div>
          </div>
          ...
          <p className="text-gray-400 text-sm font-mono leading-relaxed">{mode.description}</p>
          <div className="mt-4 text-xs text-gray-500 font-mono">› Click to select</div>
        </button>
      ))}
    </div>
  );
