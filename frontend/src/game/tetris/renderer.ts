...
export class TetrisRenderer {
  ...
  render(state: TetrisState) {
    ...
    const pushQuad = (
      ...
      for (let i = 0; i < 6; i++) { colors.push(r, g, b); alphas.push(a); }
    };

    pushQuad(boardOffX, boardOffY, this.boardPixelWidth, this.boardPixelHeight,
      BG_COLOR[0], BG_COLOR[1], BG_COLOR[2], 1.0);

    for (let row = 0; row <= BOARD_ROWS; row++) {
      pushQuad(boardOffX, boardOffY + row * cellH, this.boardPixelWidth, 1,
        0.2, 0.2, 0.25, 0.5);
    }
    for (let col = 0; col <= BOARD_COLS; col++) {
      pushQuad(boardOffX + col * cellW, boardOffY, 1, this.boardPixelHeight,
        0.2, 0.2, 0.25, 0.5);
    }
    ...
    for (let r = 0; r < BOARD_ROWS; r++) {
      for (let c = 0; c < BOARD_COLS; c++) {
        ...
        pushQuad(px, py, cellW * (1 - 2 * GAP), cellH * (1 - 2 * GAP),
          color[0], color[1], color[2], 1.0);
      }
    }

    const ghostBlocks = TETROMINOES[state.currentPiece.type][state.currentPiece.rotation];
    ...
    for (const [br, bc] of ghostBlocks) {
      ...
      const py = boardOffY + r * cellH + GAP * cellH;
      pushQuad(px, py, cellW * (1 - 2 * GAP), cellH * (1 - 2 * GAP),
        ghostColor[0], ghostColor[1], ghostColor[2], GHOST_ALPHA);
    }

    const curBlocks = TETROMINOES[state.currentPiece.type][state.currentPiece.rotation];
    ...
    pushQuad(previewX - 5, previewY - 5, this.previewSize + 10, this.previewSize + 10,
      BG_COLOR[0], BG_COLOR[1], BG_COLOR[2], 0.8);
    ...
    for (const [br, bc] of nextBlocks) {
      ...
      pushQuad(px, py, prevCellSize * (1 - 2 * GAP), prevCellSize * (1 - 2 * GAP),
        nextColor[0], nextColor[1], nextColor[2], 1.0);
    }
