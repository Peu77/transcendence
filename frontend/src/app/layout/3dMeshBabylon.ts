...
export function create3DMesh(container: HTMLElement) {
  ...
  const xAxisColors = Array(xAxisLines.length).fill(
    Array(pointsPerLine).fill(green),
  );
  const zAxisColors = Array(zAxisLines.length).fill(
    Array(pointsPerLine).fill(green),
  );

  const xAxisSystem = MeshBuilder.CreateLineSystem(
    "xLines",
    {
      lines: xAxisLines,
      updatable: true,
  ...
  const zAxisSystem = MeshBuilder.CreateLineSystem(
    "zLines",
    {
      lines: zAxisLines,
      updatable: true,
    ...
    const updateWave = (lines: Vector3[][]) => {
      for (let i = 0; i < lineCount; i++) {
        for (let j = 0; j < pointsPerLine; j++) {
          ...
          point.y =
            Math.sin(dist * waveFrequency - time * waveSpeed) * waveHeight;
        }
      }
    };
